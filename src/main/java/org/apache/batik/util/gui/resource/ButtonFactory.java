 ...
 * @version $Id: ButtonFactory.java,v 1.1 2006/05/16 14:55:33 oliver Exp $
 */
public class ButtonFactory extends ResourceManager {
    ...
    public JToggleButton createJToggleButton(String name)
	throws MissingResourceException,
	       ResourceFormatException,
	       MissingListenerException {
	...
	try {
		...
		b.setToolTipText(s);
         ...
         // Command String
        try {
            b.setActionCommand(getString(name+COMMAND_SUFFIX));
        } catch (MissingResourceException e) {
        }
        
         // is the item enabled?
	try {
	    b.setEnabled(getBoolean(name+ENABLED_SUFFIX));
	} catch (MissingResourceException e) {
	}
    }
}
